...
impl Inbox for ApActivity {
    async fn inbox<C: DbRunner>(
        ...
        match self {
            ...
            ApActivity::Reject(reject) => (*reject).inbox(conn, state, raw).await,
    ...
    fn actor(&self) -> ApAddress {
        match self {
            ...
            ApActivity::Reject(reject) => reject.actor.clone(),
...
impl Outbox for ApActivity {
    async fn outbox<C: DbRunner>(
        ...
        match self {
            ...
            ApActivity::Reject(_reject) => Err(StatusCode::NOT_IMPLEMENTED),
