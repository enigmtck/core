-- This query is optimized for public timelines WITH hashtag filters.
-- It relies on the GIN indexes on `ap_to` and `cc`, and the B-Tree index
-- `idx_activities_public_timeline` for high performance.
WITH top_activities AS (
    ...
    FROM (
        ...
        WHERE
            ...
            AND o.ek_hashtags ?| $5 -- hashtags
            --AND COALESCE(o.as_content, '') !~* ('\\m#?(' || $1 || ')\\M') -- excluded_words
            AND COALESCE(o.as_content, '') !~* ('[[:<:]]#?(' || $1 || ')[[:>:]]') 
            AND ($2::boolean = false OR o.ek_uuid IS NOT NULL) -- is_local_view
            AND (CASE WHEN $6 <> 'NULL' THEN a.created_at < $6::timestamptz ELSE TRUE END) -- max_date
            AND (CASE WHEN $7 <> 'NULL' THEN a.created_at > $7::timestamptz ELSE TRUE END) -- min_date
...
announced AS (
    ...
    LEFT JOIN activities a ON (
        ...
        AND NOT a.revoked
        AND a.kind = 'announce'
        AND (CASE WHEN $10 <> 'NULL' THEN a.actor_id = $10::integer ELSE FALSE END)
    )
...
attributed_actors AS (
    WITH unnested_ids AS (
        -- First, extract actor IDs from the string case for our limited main activities
        ...
        -- Then, extract actor IDs from the array case for our limited main activities
...
ORDER BY
    CASE WHEN $8 THEN m.created_at END ASC,
    CASE WHEN NOT $8 THEN m.created_at END DESC;
...
-- 8: order_asc (Boolean)
-- 9: limit (Integer)
-- 10: profile_actor_id (Text)

-- Example 1: Global Timeline (Unauthenticated)
-- \bind '' FALSE '{"https://www.w3.org/ns/activitystreams#Public","as:Public","Public"}' '{}' '{"#activitypub"}' NULL NULL FALSE 20 NULL
-- \g



-- Example 2: Local Timeline (Authenticated as user 7)
-- \bind '' TRUE '{"https://www.w3.org/ns/activitystreams#Public","as:Public","Public"}' '{}' '{"#activitypub"}' NULL NULL FALSE 20 7
-- \g
