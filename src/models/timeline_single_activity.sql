WITH main AS (
    ...
    WHERE
        (NULLIF($1, 'NULL')::text IS NOT NULL AND a.ap_id = $1)
        OR (NULLIF($2, 'NULL')::text IS NOT NULL AND a.uuid = $2)
        OR ($3 <> 'NULL' AND a.id = $3::integer)
),
announced AS (
    SELECT
        m.id,
    ...
    LEFT JOIN activities a ON (
        -- Param 3: profile_actor_id
        NULLIF($4, 'NULL')::integer IS NOT NULL
        AND a.target_ap_id = m.object_as_id
        AND NOT a.revoked
        AND a.kind = 'announce'
        AND (CASE WHEN $4 <> 'NULL' THEN a.actor_id = $4::integer ELSE FALSE END)
    )
...
-- 
-- PARAMETER ORDER:
-- 1: ap_id (Text)            - Use NULL if querying by id or uuid
-- 2: uuid (Text)             - Use NULL if querying by ap_id or id
-- 3: id (Integer)            - Use NULL if querying by ap_id or uuid
-- 4: profile_actor_id (Integer) - The logged-in user's ID, or NULL if unauthenticated


-- Example 1: Unauthenticated fetch by ap_id
-- \bind 'https://mastodon.social/users/reiver/statuses/114939906078715683/activity' NULL NULL NULL
-- \g

-- Example 2: Authenticated as user 7, fetch by ap_id
-- \bind 'https://mastodon.social/users/reiver/statuses/114939906078715683/activity' NULL NULL 7
-- \g

-- Example 3: Authenticated as user 7, fetch by internal id
-- \bind NULL NULL 1862070 7
-- \g

-- Example 4: Authenticated as user 7, fetch by uuid
-- \bind NULL '32ad2667-65a3-40c3-9e2c-75bfa43090e6' NULL 7
-- \g
